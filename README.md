...
- 读  : ls 命令能看到文件
- 写 : 能创建或删除文件夹下文件 或 修改文件夹/文件名
- 可执行 : 能cd 进目录

所以你需要有可读和可执行权限才能读到文件信息
...
3. **线程切换，进程切换，协程切换的不同**
   ...
   协程切换，一般只涉及CPU的上下文保存，一些寄存器信息
   
   线程切换，除了CPU的上下文信息，因为线程的调度需要内核态，所以会涉及掉特权模式的转换
   
   进程切换，因为进程有自己独立的内存地址空间，所以进程切换需要切换内存空间，所以页表缓存tlb失效，页表查询更慢
   
3. **进程运行的过程**

   - 预处理 编译 汇编 链接
   - 在就续队列 等待时间片分配， 进入执行态，需要进行进程切换，CPU上下文，内存空间切换导致tlb失效， 当访问到不存在物理内存中的块时，会发出缺页中断 通知内核态  通过页面置换算法，将需要页面置换进来，再重新执行失败语句 
   - 需要从磁盘读数据时，会采取预读取，因为磁盘顺序IO速度比随机IO速度快
   - 当CPU从内存读数据时，因为CPU和内存的反应相差很多，所以会用到高速缓存，减少CPU等待时间，一般会有3级缓存，越靠近CPU 速度越快，空间越小
   - 当程序有fork时，子进程会共享父进程资源，采取写时复制的策略，只有子进程要写的数据才会复制一份，减少空间的使用
...
14. **I/O复用 select和epoll的区别**
    ...
    **select** ①每次都要将整个fd_set数组从应用进程缓冲区复制到内核缓冲区，这是很慢的(  CPU要在用户态和核心态之间切换，堆栈需要恢复上下文)
    ...
    fd_set数组不能重用，每次传1024的bitmap ,例如 要监听 1，3，6的文件描述符， 还是传了1024的bitmap和 最大文件描述符+1 即 7，内核监听 0 - 6的文件描述符，然后返回1024的bitmap给用户态，用户态轮训 1 3 6位的状态，检查是否被置为1
...
15. **水平触发和边缘触发**
    ...
    边缘模式下，只有当缓冲区中数据由无到有，由少变多时才会进行读取数据

    边缘触发减少同一事件的重复触发次数，因此效率比水平触发高，边缘触发必须以非阻塞轮询的方式进行读取数据
